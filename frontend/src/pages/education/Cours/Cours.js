...
const Cours = () => {
  ...
  const fetchFacets = async () => {
    try {
      const response = await coursAPI.getFacets();
      if (response.data) {
  ...
  const filterCours = () => {
    ...
    if (filters.langue) {
      filtered = filtered.filter(cours => {
        const langue = cours.langueCours?.toLowerCase() || '';
        return langue.includes(filters.langue.toLowerCase());
      });
    }
  ...
  const showCoursDetails = async (cours) => {
    try {
      ...
      // Merge API response with original cours data to ensure all fields are available
      const mergedData = { ...cours, ...details };
      setSelectedCours(mergedData);
  ...
  const fetchDBpediaEnrichment = async (coursId, term = null) => {
    try {
      setLoadingDBpedia(true);
      setDbpediaData(null);
      const response = await coursAPI.enrichWithDBpedia(coursId, term);
      if (response.data) {
  ...
  return (
    <div className="cours-container">
        ...
        <div className="filters-grid">
          <div className="filter-group">
            ...
            >
              ...
              {facets.by_semestre && facets.by_semestre.map((facet, index) => (
                <option key={index} value={facet.semestre || ''}>
                  {facet.semestre || 'Non spécifié'} ({facet.count || 0})
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="filter-group">
            ...
            >
              ...
              {facets.by_langue && facets.by_langue.map((facet, index) => (
                <option key={index} value={facet.langueCours || ''}>
                  {facet.langueCours || 'Non spécifié'} ({facet.count || 0})
                </option>
              ))}
            </select>
          </div>
      ...
      {showDetailsModal && selectedCours && (
        <div 
        ...
        >
          ...
          >
            <div className="modal-header">
              <h2>{selectedCours.intitule || 'Détails du cours'}</h2>
              <button 
            ...
            <div className="modal-body">
              {activeTab === 'info' && selectedCours && (
                <div className="tab-content">
                  <div className="detail-section">
                    <h3>Informations générales</h3>
                    <div className="detail-grid">
                      {coursDetailsFields.map((field) => {
                        const value = selectedCours[field.name];
                              ...
                              {value && value.length < 100 && (
                                <button 
                                  ...
                                  onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    fetchDBpediaEnrichment(selectedCours.cours, value);
                                  }}
                  ...
                  {loadingDBpedia ? (
                    ...
                    <div className="no-dbpedia">
                      ...
                      <button 
                        ...
                        onClick={() => selectedCours && fetchDBpediaEnrichment(selectedCours.cours, selectedCours.intitule)}
                      >
                        🔄 Charger les données DBpedia pour "{selectedCours?.intitule || 'ce cours'}"
                      </button>
                    </div>
                  )}
                </div>
